...
public class GuestBookController {
	...
	public String getArticle(@PathVariable int articleno, Model model) {
		...
		try {
			dto = service.getArticle(articleno);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String writeArticle(GuestBookDto guestBookDto, Model model) {
        ...
        int result = 0;
		try {
			result = service.writeArticle(guestBookDto);
		} catch (Exception e) {
			e.printStackTrace();
		}
        if(result > 0) {
	...
	public String updateArticle(GuestBookDto guestBookDto, Model model) {
		...
		int result = 0;
		try {
			result = service.updateArticle(guestBookDto);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public String deleteArticle(@PathVariable int articleno, Model model) {
		...
		int result = 0;
		try {
			result = service.deleteArticle(articleno);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
